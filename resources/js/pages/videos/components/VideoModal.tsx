    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (isEditMode) {
            ...
            post(route('videos.update', video!.id), {
                _method: 'put', // Laravel expects this for PUT in FormData
                forceFormData: true,
                onSuccess: () => {
                    Swal.fire({
                        title: 'Updated!',
            ...
            <DialogContent className="max-w-lg border border-slate-200 bg-white/90 ring-1 ring-slate-200/60 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:border-slate-800/70 dark:bg-slate-900/80 dark:ring-slate-800/50">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-slate-600 dark:text-slate-400">
                        {isEditMode
                            ? 'Update video details. Leave the file field blank to keep the current file.'
                            : 'Upload a new video file. Fields marked with (*) are required.'}
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit}>
                    <Box className="grid gap-4 py-4">
                        ...
                        <Box className="grid grid-cols-4 items-center gap-4">
                            ...
                            />
                            {errors.file_path && (
                                <p className="col-span-4 text-red-500 text-sm">{errors.file_path}</p>
                            )}
                        </Box>
                    </Box>
